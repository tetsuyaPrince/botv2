...
===✒️ All Cmd ✒️===
||=========================||
||✒️ Mention/Tagall  - Mention All
||✒️ Point   - Set Point
||✒️ Read    - Check Read
||✒️ Ginfo   - Group Info
||✒️ Glist  - Ur Group List
||✒️ Cancel - Cancel Inv
||✒️ Mid - Get Ur Mid
||✒️ Mid @  - Get Mid
||✒️ Invite - Inv by Cont
||✒️ Invite: - Inv by Mid
||✒️ Link on  - Open Link
||✒️ Link off - Close Link
||✒️ Gurl - Open n Get Link
||✒️ Url - Get Link
||✒️ Gname: - Set Group Name
||✒️ Reject - Cancel all Inv
||✒️ Spam on/off [time] [text]
||✒️ Shup - Spam Shut up
||✒️ Allname: - Set Name
||✒️ Allbio:  - Set Bio
||✒️ Copy @ - Copy Profile
||✒️ Backup @  - Backup Profile 
||✒️ Kick @ - Kick user
||===== ✯ Setting ✯ =====||
||✒️ Set - View Status
||✒️ [Add on/off] - Auto msg set 
||✒️ [Join on/off] - Auto Join	   	
||✒️ [Message set:] - Add msg set		
||===== ✯ Protect ✯ =====||       
||✒️ [Panick:on/off] - All     
||✒️ [Protect on]			   
||✒️ [Qrprotect on/off]			   
...
# ----------------- NOTIFED MEMBER OUT GROUP
        if op.type == 15:
            if op.param2 in bot1:
                return
            cl.sendText(op.param1,"Good Bye " + cl.getContact(op.param2).displayName + " See you next time")
            print ("MEMBER HAS LEFT THE GROUP")
#------------------ KICK OUT FORM GROUP
        if op.type == 19:
            if op.param2 in bot1:
                return
            cl.sendText(op.param1,cl.getContact(op.param2).displayName + " Why do you do like that")
...
# ----------------- NOTIFED MEMBER JOIN GROUP
        if op.type == 17:
            ...
            cl.sendText(op.param1, "Welcome " + cl.getContact(op.param2).displayName + " to\nGroup " + "👉" + str(ginfo.name) + "👈""\n\nRespect\nMinimal Crew 2017")
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                ...
                if wait["inviteprotect"] == True: md+="☞ Invite Protect → ✔\n"
                ...
                if msg.toType == 2:
                    cl.sendText(msg.to, "Set Point:" + datetime.now().strftime('\n📅%Y/%m/%d 🕛 %H:%M:%S'))
                    try:
                ...
                if msg.toType == 2:
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "==============================\nActive readers:%s\n\n\n\nPassive readers:\n%s\n\n==============================\nIn the last seen point:\n[%s]\n==============================\n Powered By: Jaylien" % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                        print "ReadPoint Set..."
                        try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
...
#-----------------------------------------------------------
   ...
   #----------------------------------------------------------------
                ...
                if wait["lang"] == "JP":
                    ki6.sendText(msg.to,"Reject All Success")
                else:
...
#----------------------ADMIN COMMAND------------------------------#
                ...
                if msg.from_ in admin:
                    ...
                    if gid in [""," "]:
                        cl.sendText(msg.to,"Group ID Inviald")
                    else:
                        try:
                ...
                if msg.toType == 2:
                    ...
                    cl.sendText(msg.to,"Can't Change Group Name")
              ...
              if msg.from_ in admin:
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    cl.sendText(msg.to, "Copy Success")
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    ki.sendText(msg.to, "Copy Success")
                                except Exception as e:
                                    print e

            elif msg.text in ["ออกร่าง","กลับร่าง","คืนร่าง"]:
                try:
                    cl.updateDisplayPicture(mybackup.pictureStatus)
                    cl.updateProfile(mybackup)
                    cl.sendText(msg.to, "Backup Success")
                except Exception as e:
                ...
                try:
                    ...
                    ki.sendText(msg.to, "Backup Success")
                except Exception as e:
              ...
              if msg.from_ in owner:
                ...
                cl.sendText(msg.to, "Speed is" % (elapsed_time))
              ...
              if msg.from_ in admin:
                ...
                if wait["lang"] == "JP":
                    ...
                    else: md += "\nMembers : " + str(len(group.members)) + " People" + "\nInviting : " + str(len(group.invitee)) + " People"
                    cl.sendText(msg.to,md)
...
#------------------------------------------------------------------	
 ...
 #------------------------------------------------------------------
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                cl.sendText(msg.to,"ไม่มีชื่อในลิสต์แบน")
            ...
            elif msg.text in ["Clear ban","ล้างแบน"]:
              if msg.from_ in admin:
                wait["blacklist"] = {}
                cl.sendText(msg.to,"ล้างแบนสำเร็จ")
            elif msg.text in ["Whitelist","ปลดแบน"]:
              if msg.from_ in admin:
                wait["wblacklist"] = True
                cl.sendText(msg.to,"ส่งคอนแทคเพื่อปลดแบน")
            
            elif msg.text in ["Blacklist","แบน"]:
              if msg.from_ in admin:
                wait["dblacklist"] = True
                cl.sendText(msg.to,"ส่งคอนแทคเพื่อแบน")
            
            elif msg.text in ["Banlist","เชคแบน","เช็คแบน"]:
              if msg.from_ in admin:
                if wait["blacklist"] == {}:
                    cl.sendText(msg.to,"ไม่มีคนโดนแบน")
                else:
                    cl.sendText(msg.to,"รายชื่อแบน􏿿")
                    mc = "[⎈]Blacklist [⎈]\n"
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    for tag in wait["blacklist"]:
                        matched_list+=filter(lambda str: str == tag, gMembMids)
                    if matched_list == []:
                        ki.sendText(msg.to,"ไม่มีชื่อในแบนลิสต์")
                        return
